#!/usr/bin/env python3
"""Verify selector-v17 sealing before opening a comparison target."""
...
def verify_v17_seal(manifest_path: Path, sealed_dir: Path) -> tuple[dict, dict]:
    manifest_path, sealed_dir = manifest_path.resolve(), sealed_dir.resolve()
    manifest = json.loads(manifest_path.read_text())
    if manifest.get("schema") != "fold-protein-blind-selector/v17":
        raise RuntimeError("invalid v17 manifest; target access forbidden")
    seal_path = sealed_dir / "seal.json"
    if not seal_path.is_file():
        raise RuntimeError("missing v17 seal; target access forbidden")
    seal = json.loads(seal_path.read_text())
    if (seal.get("schema") != "fold-protein-blind-seal/v17"
            or seal.get("status") != "completed"):
        raise RuntimeError("invalid v17 seal; target access forbidden")
    for field, path in {
        ...
        if seal.get(field) != sha256(path):
            raise RuntimeError(f"v17 seal mismatch for {field}")
    ...
    if (states.get("sequence") != sequence
        ...
        raise RuntimeError("v17 input/state identity mismatch")
    if hashlib.sha256(sequence.encode()).hexdigest() != seal.get("sequence_sha256"):
        raise RuntimeError("v17 sequence hash mismatch")
    if len(states.get("orientation_trace", [])) != max(0, len(sequence) - 3):
        raise RuntimeError("v17 orientation trace mismatch")
    for field in (
        ...
        if states.get(field) != seal.get(field):
            raise RuntimeError(f"v17 {field} mismatch")
    if len(states.get("final_relations", {}).get("objectives", [])) != \
            seal.get("constitutional_objectives"):
        raise RuntimeError("v17 objective census mismatch")
    for relative, expected in seal.get("source_sha256", {}).items():
        if (manifest["source_sha256"].get(relative) != expected
            ...
            raise RuntimeError(f"v17 source mismatch for {relative}")
    return seal, states


def evaluate_v17(manifest_path: Path, sealed_dir: Path, target_path: Path,
                 output_path: Path) -> dict:
    seal, states = verify_v17_seal(manifest_path, sealed_dir)
    ...
    result = {
        "schema": "fold-protein-blind-evaluation/v17",
        "status": "completed",
