#!/usr/bin/env python3
"""Verify and evaluate sealed selector V23.2."""
...
def verify(manifest_path, sealed_dir):
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v23.2" or seal.get("schema") != "fold-protein-blind-seal/v23.2":
        raise RuntimeError("invalid v23.2 seal; target access forbidden")
    for field, path in {"protocol_manifest_sha256": manifest_path, "selector_input_sha256": sealed_dir/"selector_input.json", "selected_states_sha256": sealed_dir/"selected_states.json", "prediction_pdb_sha256": sealed_dir/"prediction.pdb"}.items():
        if seal.get(field) != sha(path): raise RuntimeError(f"v23.2 seal mismatch: {field}")
    states = json.loads((sealed_dir/"selected_states.json").read_text())
    if len(states["states"]) != len(states["sequence"]) or any(row["expanded_state_count"] != 576 for row in states["domain_trace"]):
        raise RuntimeError("v23.2 domain/state closure mismatch")
    seed = manifest["seed_records"][seal["seed_run_id"]]
    if sha(ROOT/seed["path"]) != seed["sha256"]: raise RuntimeError("v23.2 seed drift")
    for relative, expected in manifest["source_sha256"].items():
        if sha(ROOT/relative) != expected: raise RuntimeError(f"v23.2 source drift: {relative}")
    return seal, states
