#!/usr/bin/env python3
"""Post-seal same-index local evaluation for selector v12."""
...
from verify.evaluate_sealed_blind_local_v11 import (  # noqa: E402
    distance_rmsd, kabsch_rmsd)
from verify.evaluate_sealed_blind_v12 import (  # noqa: E402
    sha256, verify_v12_seal)
...
def evaluate(manifest: Path, sealed_dir: Path, target_path: Path,
    ...
    seal, states = verify_v12_seal(manifest, sealed_dir)
    ...
    maximum_tm = max(windows, key=lambda row: (
        row["local_tm_score"],
        [-value for value in row["residue_positions_one_based"]]))
    result = {
        "schema": "fold-protein-blind-local-evaluation/v12",
        "status": "completed",
        "run_id": seal["run_id"],
        ...
        "provenance": {
            ...
            "authority": "Maria Smith owns publishable conclusions and direction.",
        },
        "source_sha256": {
...
def main() -> None:
    ...
    result = evaluate(args.manifest, args.sealed_dir, args.target, args.output,
                      args.window_length)
    print(json.dumps({
