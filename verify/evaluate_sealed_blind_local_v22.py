#!/usr/bin/env python3
"""Post-seal same-index local evaluation for selector v22."""
...
from verify.evaluate_sealed_blind_v22 import sha256, verify_v22_seal  # noqa: E402
...
def evaluate(manifest: Path, sealed_dir: Path, target_path: Path,
    ...
    seal, states = verify_v22_seal(manifest, sealed_dir)
    ...
    result = {
        "schema": "fold-protein-blind-local-evaluation/v22",
        "status": "completed", "result_type": "cumulative development benchmark",
        "official_run": False, "run_id": seal["run_id"],
        ...
        "matched_ca_atoms": matched, "window_length_residues": window_length,
        "window_count": len(windows),
        "window_rule": "After complete seal verification, report every same-index contiguous window; no target measurement enters selection.",
...
def main() -> None:
    ...
    result = evaluate(
        ...
        args.window_length)
    print(json.dumps({
        "status": result["status"], "window_count": result["window_count"],
        "minimum_kabsch_rmsd_window": result["minimum_kabsch_rmsd_window"],
        "maximum_local_tm_window": result["maximum_local_tm_window"]}, sort_keys=True))
