...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Protein canonical-angle admission ===");
    admitted = enforce_canonical_angle_form_uniqueness();
    if (admitted == 1) {
    printf("%s\n", (char*)(long long)"  ok    all four canonical angle forms are uniquely admitted");
    ret_val = 0;
    goto L_cleanup;
    }
    ret_val = halt_violation((long long)"canonical Protein angle admission returned without closure");
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
