#!/usr/bin/env python3
"""Verify and evaluate sealed coordinate-beam selector V25."""
...
def verify(manifest_path, sealed_dir):
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v25" or seal.get("schema") != "fold-protein-blind-seal/v25":
        raise RuntimeError("invalid V25 seal; target access forbidden")
    for field,path in {"protocol_manifest_sha256":manifest_path,"selector_input_sha256":sealed_dir/"selector_input.json","selected_states_sha256":sealed_dir/"selected_states.json","prediction_pdb_sha256":sealed_dir/"prediction.pdb"}.items():
        if seal.get(field) != sha(path): raise RuntimeError(f"V25 seal mismatch: {field}")
    states=json.loads((sealed_dir/"selected_states.json").read_text())
    if len(states["states"]) != len(states["sequence"]) or not all(row["retained_paths"] == 24 for row in states["assembly_trace"]):
        raise RuntimeError("V25 coordinate beam closure mismatch")
    parent=manifest["parent_records"][seal["parent_run_id"]]
    if sha(ROOT/parent["path"]) != parent["sha256"]: raise RuntimeError("V25 parent drift")
    for relative,expected in manifest["source_sha256"].items():
        if sha(ROOT/relative) != expected: raise RuntimeError(f"V25 source drift: {relative}")
    return seal,states
