...
long long expect_bool(long long label, long long got) {
    ...
    halted = halt_violation(concat((long long)"protein window closure failed: ", label));
    }
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== protein colour-window orientation ===");
    ok = expect_bool((long long)"window is the fold-derived colour count", protein_orientation_window_closes());
    ok = expect_bool((long long)"overlap is the fold-derived binary count", protein_orientation_overlap_closes());
    ok = expect_bool((long long)"complete overlap space has one candidate with One advance", protein_orientation_overlap_candidates() == 1);
    ok = expect_bool((long long)"each overlapping window advances by the One", protein_orientation_advance_is_the_one());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
