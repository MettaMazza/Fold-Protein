#!/usr/bin/env python3
"""Verify a selector-v2 seal completely before opening a comparison target."""
...
def verify_v2_seal(manifest_path: Path, sealed_dir: Path) -> tuple[dict, dict]:
    manifest_path = manifest_path.resolve()
    sealed_dir = sealed_dir.resolve()
    manifest = json.loads(manifest_path.read_text())
    if manifest.get("schema") != "fold-protein-blind-selector/v2":
        raise RuntimeError("invalid v2 manifest; target access forbidden")
    seal_path = sealed_dir / "seal.json"
    if not seal_path.is_file():
        raise RuntimeError("missing v2 seal; target access forbidden")
    seal = json.loads(seal_path.read_text())
    if seal.get("schema") != "fold-protein-blind-seal/v2" or \
            seal.get("status") != "completed":
        raise RuntimeError("invalid or incomplete v2 seal; target access forbidden")
    required = {
    ...
    for field, actual in required.items():
        if seal.get(field) != actual:
            raise RuntimeError(f"v2 seal mismatch for {field}; target access forbidden")
    ...
    if states.get("sequence") != sequence or states.get("run_id") != selector_input.get("run_id"):
        raise RuntimeError("v2 input/state identity mismatch; target access forbidden")
    if hashlib.sha256(sequence.encode()).hexdigest() != seal.get("sequence_sha256"):
        raise RuntimeError("v2 sequence hash mismatch; target access forbidden")
    if len(states.get("states", [])) != seal.get("path_length") or \
            seal.get("path_length") != len(sequence):
        raise RuntimeError("v2 path length mismatch; target access forbidden")
    for relative, expected in seal.get("source_sha256", {}).items():
        if manifest.get("source_sha256", {}).get(relative) != expected or \
            ...
            raise RuntimeError(f"v2 source mismatch for {relative}; target access forbidden")
    return seal, states


def evaluate_v2(manifest_path: Path, sealed_dir: Path, target_path: Path,
                output_path: Path) -> dict:
    seal, states = verify_v2_seal(manifest_path, sealed_dir)

    # No target filesystem operation occurs before verify_v2_seal returns.
    ...
    evidence = {
        "schema": "fold-protein-blind-evaluation/v2",
        "status": "completed",
        "run_id": seal["run_id"],
    ...
    output_path.resolve().write_text(json.dumps(evidence, indent=2, sort_keys=True) + "\n")
    return evidence
