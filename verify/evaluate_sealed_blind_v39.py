#!/usr/bin/env python3
"""Verify the V39 causal seal before opening a comparison target."""
...
def verify_v39_seal(manifest_path: Path, sealed_dir: Path) -> tuple[dict, dict]:
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v39":
        raise RuntimeError("invalid V39 manifest; target access forbidden")
    seal_path = sealed_dir / "seal.json"
    if not seal_path.is_file():
        raise RuntimeError("missing V39 seal; target access forbidden")
    seal = json.loads(seal_path.read_text())
    if (seal.get("schema") != "fold-protein-blind-seal/v39"
            or seal.get("status") != "completed"):
        raise RuntimeError("invalid V39 seal; target access forbidden")
    required = {
    ...
    for field, actual in required.items():
        if seal.get(field) != actual:
            raise RuntimeError(f"V39 seal mismatch: {field}; target access forbidden")
    if seal.get("parent_v38") != manifest.get("parent_v38"):
        raise RuntimeError("V39 parent binding mismatch; target access forbidden")
    ...
    if (states.get("sequence") != sequence
            ...
            or states.get("parent_fixed_point_count") != 4
            or states.get("causal_direction") != "n_to_c"
            or states.get("causal_axis_order") != [0, 1]
            or states.get("causal_event_ranks") != {"phi": 3, "psi": 4}):
        raise RuntimeError("V39 peptide-causal identity mismatch")
    if any(not 0 <= state < 576 for state in states["states"]):
        raise RuntimeError("V39 emitted a state outside the complete paired lattice")
    for relative, expected in manifest["source_sha256"].items():
        if seal["source_sha256"].get(relative) != expected or sha256(ROOT / relative) != expected:
            raise RuntimeError(f"V39 source drift: {relative}; target access forbidden")
    return seal, states


def evaluate_v39(manifest_path: Path, sealed_dir: Path, target_path: Path,
                 output_path: Path) -> dict:
    seal, states = verify_v39_seal(manifest_path, sealed_dir)
    ...
    evidence = {
        "schema": "fold-protein-blind-evaluation/v39",
        "status": "completed",
        ...
        "states_changed_from_v38_emission": sum(
            left != right for left, right in zip(states["states"], states["parent_selected_states"])),
        "causal_direction": states["causal_direction"],
        "causal_axis_order": states["causal_axis_order"],
        "causal_event_ranks": states["causal_event_ranks"],
        "selected_parent_sweeps": states["selected_parent_sweeps"],
        "selected_parent_evaluations": states["selected_parent_evaluations"],
    }
