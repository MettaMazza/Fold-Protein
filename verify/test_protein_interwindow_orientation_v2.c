...
long long _main();
long long protein_local_plane_window();
long long protein_plane_overlap();
long long protein_interwindow_one_count();
long long protein_orientation_quartet();
long long protein_orientation_quartet_overlap();
long long protein_orientation_quartet_advance();
long long protein_orientation_quartet_overlap_candidates();
long long protein_orientation_quartet_overlap_is_unique();
long long protein_orientation_quartet_closes();
...
long long expect_bool(long long label, long long got) {
    ...
    halted = halt_violation(concat((long long)"protein inter-window closure failed: ", label));
    }
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== protein inter-window orientation ===");
    ok = expect_bool((long long)"two colour windows minus binary overlap form four residues", protein_orientation_quartet_closes());
    ok = expect_bool((long long)"orientation quartets overlap by one colour window", protein_orientation_overlap_closes());
    ok = expect_bool((long long)"complete quartet-overlap space has one candidate with One advance", protein_orientation_quartet_overlap_candidates() == 1);
    ok = expect_bool((long long)"each orientation quartet advances by the One", protein_orientation_advance_is_the_one());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long protein_orientation_quartet() {
    long long from_binary_square = 0;
    long long from_union = 0;
    long long ret_val = 0;

    ep_gc_push_root(&from_binary_square);
    ep_gc_push_root(&from_union);

    ep_gc_maybe_collect();

    from_union = ((smallest_fold_period_above(binary_count()) + smallest_fold_period_above(binary_count())) - smallest_fold_period_above(1));
    from_binary_square = (smallest_fold_period_above(1) * smallest_fold_period_above(1));
    ret_val = forced_to_be((long long)"two colour windows with binary overlap form the binary square", from_union, from_binary_square);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long protein_orientation_quartet_advance() {
    ...
    from_overlap = (protein_orientation_quartet() - smallest_fold_period_above(binary_count()));
    from_the_one = fold_period(the_one(), binary_count());
    ret_val = forced_to_be((long long)"orientation quartet advance", from_overlap, from_the_one);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long protein_orientation_quartet_overlap_candidates() {
    ...
    quartet = protein_orientation_quartet();
    one = fold_period(the_one(), binary_count());
    matching = 0;
    candidate = 0;
    while (candidate <= quartet) {
...
long long protein_orientation_quartet_overlap_is_unique() {
    ...
    quartet = protein_orientation_quartet();
    overlap = smallest_fold_period_above(binary_count());
    one = fold_period(the_one(), binary_count());
    claimed_satisfies = 0;
    if ((quartet - overlap) == one) {
    claimed_satisfies = 1;
    }
    alternatives = (protein_orientation_quartet_overlap_candidates() - 1);
    guarded = forced_unique((long long)"colour count is the unique quartet overlap with One advance", claimed_satisfies, alternatives);
    independently_forced = (quartet - one);
    closed_overlap = forced_to_be((long long)"quartet overlap equals quartet less the One", overlap, independently_forced);
    if (guarded == 1) {
...
long long protein_orientation_quartet_closes() {
    ...
    ret_val = protein_orientation_quartet() == (smallest_fold_period_above(1) * smallest_fold_period_above(1));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long protein_orientation_overlap_closes() {
    ...
    ret_val = protein_orientation_quartet_overlap_is_unique();
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long protein_orientation_advance_is_the_one() {
    ...
    ret_val = protein_orientation_quartet_advance() == fold_period(the_one(), binary_count());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
