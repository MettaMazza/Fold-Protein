#!/usr/bin/env python3
"""Verify the V36 seal before opening a comparison target."""
...
def verify_v36_seal(manifest_path: Path, sealed_dir: Path) -> tuple[dict, dict]:
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v36":
        raise RuntimeError("invalid V36 manifest; target access forbidden")
    seal_path = sealed_dir / "seal.json"
    if not seal_path.is_file():
        raise RuntimeError("missing V36 seal; target access forbidden")
    seal = json.loads(seal_path.read_text())
    if (seal.get("schema") != "fold-protein-blind-seal/v36"
            or seal.get("status") != "completed"):
        raise RuntimeError("invalid V36 seal; target access forbidden")
    required = {
    ...
    for field, actual in required.items():
        if seal.get(field) != actual:
            raise RuntimeError(f"V36 seal mismatch: {field}; target access forbidden")
    ...
    if (states.get("sequence") != sequence
            ...
            or states.get("chain_boundaries") != 2
            or states.get("boundary_contexts") != 8
            or states.get("complete_chain_candidates") != 16
            or len(states.get("reconciliation_trace", [])) != 16):
        raise RuntimeError("V36 complete-chain identity mismatch")
    if set(states["candidates_per_boundary"].values()) != {8}:
        raise RuntimeError("V36 boundary candidate coverage mismatch")
    domain = set(states["closed_domain"].values())
    if any(state not in domain for state in states["states"][:-1]):
        raise RuntimeError("V36 emitted a state outside the engine-closed domain")
    for relative, expected in manifest["source_sha256"].items():
        if seal["source_sha256"].get(relative) != expected or sha256(ROOT / relative) != expected:
            raise RuntimeError(f"V36 source drift: {relative}; target access forbidden")
    return seal, states


def evaluate_v36(manifest_path: Path, sealed_dir: Path, target_path: Path,
                 output_path: Path) -> dict:
    seal, states = verify_v36_seal(manifest_path, sealed_dir)
    ...
    evidence = {
        "schema": "fold-protein-blind-evaluation/v36",
        "status": "completed",
