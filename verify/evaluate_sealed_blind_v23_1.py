#!/usr/bin/env python3
"""Verify selector-v23.1 sealing before opening a comparison target."""
...
def verify_v23_1_seal(manifest_path: Path, sealed_dir: Path):
    ...
    if (manifest.get("schema") != "fold-protein-blind-selector/v23.1"
            or seal.get("schema") != "fold-protein-blind-seal/v23.1"
            or seal.get("status") != "completed"):
        raise RuntimeError("invalid v23.1 seal; target access forbidden")
    for field, path in {
        ...
        if seal.get(field) != sha256(path):
            raise RuntimeError(f"v23.1 seal mismatch for {field}")
    states = json.loads((sealed_dir / "selected_states.json").read_text())
    sequence = states.get("sequence", "")
    if (len(states.get("states", [])) != len(sequence)
            or hashlib.sha256(sequence.encode()).hexdigest() != seal.get("sequence_sha256")
            or states.get("parent_departures") != seal.get("parent_departures")):
        raise RuntimeError("v23.1 state identity mismatch")
    ...
    if (not seed or seed.get("sha256") != seal.get("seed_selected_states_sha256")
        ...
        raise RuntimeError("v23.1 registered seed binding mismatch")
    for relative, expected in seal.get("source_sha256", {}).items():
        if (manifest["source_sha256"].get(relative) != expected
            ...
            raise RuntimeError(f"v23.1 source mismatch for {relative}")
    return seal, states


def evaluate_v23_1(manifest_path: Path, sealed_dir: Path, target_path: Path,
                   output_path: Path) -> dict:
    seal, states = verify_v23_1_seal(manifest_path, sealed_dir)
    ...
    result = {
        "schema": "fold-protein-blind-evaluation/v23.1", "status": "completed",
        ...
        "parent_departures": states["parent_departures"],
    }
...
def main() -> None:
    ...
    print(json.dumps(evaluate_v23_1(
