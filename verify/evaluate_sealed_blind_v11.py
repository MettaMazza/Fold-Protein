#!/usr/bin/env python3
"""Verify selector-v11 sealing before opening a comparison target."""
...
def verify_v11_seal(manifest_path: Path, sealed_dir: Path) -> tuple[dict, dict]:
    manifest_path = manifest_path.resolve()
    sealed_dir = sealed_dir.resolve()
    manifest = json.loads(manifest_path.read_text())
    if manifest.get("schema") != "fold-protein-blind-selector/v11":
        raise RuntimeError("invalid v11 manifest; target access forbidden")
    seal_path = sealed_dir / "seal.json"
    if not seal_path.is_file():
        raise RuntimeError("missing v11 seal; target access forbidden")
    seal = json.loads(seal_path.read_text())
    if seal.get("schema") != "fold-protein-blind-seal/v11" or \
            seal.get("status") != "completed":
        raise RuntimeError(
            "invalid or incomplete v11 seal; target access forbidden")
    for field, path in {
        ...
        if seal.get(field) != sha256(path):
            raise RuntimeError(
                f"v11 seal mismatch for {field}; target access forbidden")
    ...
    for relative, expected in seal.get("source_sha256", {}).items():
        if manifest.get("source_sha256", {}).get(relative) != expected or \
            ...
            raise RuntimeError(
                f"v11 source mismatch for {relative}; target access forbidden")
    return seal, states


def evaluate_v11(manifest_path: Path, sealed_dir: Path, target_path: Path,
                 output_path: Path) -> dict:
    seal, states = verify_v11_seal(manifest_path, sealed_dir)
    ...
    evidence = {
        "schema": "fold-protein-blind-evaluation/v11",
        "status": "completed",
        "run_id": seal["run_id"],
