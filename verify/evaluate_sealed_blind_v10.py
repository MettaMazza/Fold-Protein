#!/usr/bin/env python3
"""Verify selector-v10 sealing before opening a comparison target."""
...
def verify_v10_seal(manifest_path: Path, sealed_dir: Path) -> tuple[dict, dict]:
    manifest_path = manifest_path.resolve()
    sealed_dir = sealed_dir.resolve()
    manifest = json.loads(manifest_path.read_text())
    if manifest.get("schema") != "fold-protein-blind-selector/v10":
        raise RuntimeError("invalid v10 manifest; target access forbidden")
    seal_path = sealed_dir / "seal.json"
    if not seal_path.is_file():
        raise RuntimeError("missing v10 seal; target access forbidden")
    seal = json.loads(seal_path.read_text())
    if seal.get("schema") != "fold-protein-blind-seal/v10" or \
            seal.get("status") != "completed":
        raise RuntimeError("invalid or incomplete v10 seal; target access forbidden")
    for field, path in {
        ...
        if seal.get(field) != sha256(path):
            raise RuntimeError(f"v10 seal mismatch for {field}; target access forbidden")
    ...
    if states.get("sequence") != sequence or \
        ...
        raise RuntimeError("v10 input/state identity mismatch; target access forbidden")
    if hashlib.sha256(sequence.encode()).hexdigest() != seal.get("sequence_sha256"):
        raise RuntimeError("v10 sequence hash mismatch; target access forbidden")
    if len(states.get("states", [])) != seal.get("path_length") or \
            seal.get("path_length") != len(sequence):
        raise RuntimeError("v10 path length mismatch; target access forbidden")
    if len(states.get("orientation_trace", [])) != max(0, len(sequence) - 3) or \
            seal.get("orientation_quartets") != max(0, len(sequence) - 3):
        raise RuntimeError("v10 orientation trace mismatch; target access forbidden")
    if states.get("orientation_modes") != seal.get("generated_orientation_modes"):
        raise RuntimeError("v10 generated-mode mismatch; target access forbidden")
    if states.get("charge_census") != seal.get("charge_census") or \
            states.get("steric_census") != seal.get("steric_census"):
        raise RuntimeError("v10 residue constitution mismatch; target access forbidden")
    if states.get("mode_capacity") != seal.get("mode_capacity") or \
            seal.get("binary_mode_count") * seal.get("mode_capacity") != 24:
        raise RuntimeError("v10 mode-capacity mismatch; target access forbidden")
    if len(states.get("final_relations", {}).get("objectives", [])) != \
            seal.get("constitutional_objectives"):
        raise RuntimeError("v10 objective census mismatch; target access forbidden")
    for row in states.get("mode_balance_trace", []):
        if set(row) != {"alpha", "beta"}:
            raise RuntimeError("v10 mode balance is incomplete; target access forbidden")
    for relative, expected in seal.get("source_sha256", {}).items():
        if manifest.get("source_sha256", {}).get(relative) != expected or \
            ...
            raise RuntimeError(f"v10 source mismatch for {relative}; target access forbidden")
    return seal, states


def evaluate_v10(manifest_path: Path, sealed_dir: Path, target_path: Path,
                 output_path: Path) -> dict:
    seal, states = verify_v10_seal(manifest_path, sealed_dir)
    ...
    evidence = {
        "schema": "fold-protein-blind-evaluation/v10",
        "status": "completed",
        "run_id": seal["run_id"],
