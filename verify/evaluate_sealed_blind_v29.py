#!/usr/bin/env python3
"""Verify and evaluate sealed tertiary segment-body selector V29."""
...
def verify(manifest_path, sealed_dir):
    ...
    if (manifest.get("schema") != "fold-protein-blind-selector/v29"
            or seal.get("schema") != "fold-protein-blind-seal/v29"):
        raise RuntimeError("invalid V29 seal; target access forbidden")
    for field, path in {
        ...
        if seal.get(field) != sha(path):
            raise RuntimeError(f"V29 seal mismatch: {field}")
    states = json.loads((sealed_dir / "selected_states.json").read_text())
    binding = manifest["parent_records"][seal["v28_run_id"]]
    for name in ("v25", "v26_1", "v27", "v28", "domain"):
        if sha(ROOT / binding[name + "_path"]) != binding[name + "_sha256"]:
            raise RuntimeError("V29 branch lineage drift")
    if (len(states["states"]) != len(states["sequence"])
            or states["tertiary_census"]["tree_edge_count"]
            != states["tertiary_census"]["segment_count"] - 1
            or any(row["retained_paths"] != 24
                   for row in states["assembly_trace"])):
        raise RuntimeError("V29 tertiary topology closure mismatch")
    for relative, expected in manifest["source_sha256"].items():
        if sha(ROOT / relative) != expected:
            raise RuntimeError(f"V29 source drift: {relative}")
    return seal, states
...
def evaluate(manifest_path, sealed_dir, target_path, output_path):
    ...
    prediction, target = (
        parse_ca(str(sealed_dir / "prediction.pdb")), parse_ca(str(target_path)))
    ...
    result = {
        "schema": "fold-protein-blind-evaluation/v29", "status": "completed",
        ...
        "tree_edge_count": states["tertiary_census"]["tree_edge_count"],
    }
