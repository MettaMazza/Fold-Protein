#!/usr/bin/env python3
"""Verify the V34 seal completely before opening a comparison target."""
...
def verify_v34_seal(manifest_path: Path, sealed_dir: Path) -> tuple[dict, dict]:
    manifest_path = manifest_path.resolve()
    sealed_dir = sealed_dir.resolve()
    manifest = json.loads(manifest_path.read_text())
    if manifest.get("schema") != "fold-protein-blind-selector/v34":
        raise RuntimeError("invalid V34 manifest; target access forbidden")
    seal_path = sealed_dir / "seal.json"
    if not seal_path.is_file():
        raise RuntimeError("missing V34 seal; target access forbidden")
    seal = json.loads(seal_path.read_text())
    if (seal.get("schema") != "fold-protein-blind-seal/v34"
            or seal.get("status") != "completed"):
        raise RuntimeError("invalid V34 seal; target access forbidden")
    required = {
    ...
    for field, actual in required.items():
        if seal.get(field) != actual:
            raise RuntimeError(f"V34 seal mismatch: {field}; target access forbidden")
    ...
    if (states.get("sequence") != sequence
            ...
            or states.get("closed_domain") != manifest["selector_config"]["candidate_domain"]):
        raise RuntimeError("V34 closed-domain state identity mismatch")
    domain = set(states["closed_domain"].values())
    if any(state not in domain for state in states["states"][:-1]):
        raise RuntimeError("V34 emitted a state outside the engine-closed domain")
    for relative, expected in manifest["source_sha256"].items():
        if seal["source_sha256"].get(relative) != expected or sha256(ROOT / relative) != expected:
            raise RuntimeError(f"V34 source drift: {relative}; target access forbidden")
    return seal, states


def evaluate_v34(manifest_path: Path, sealed_dir: Path, target_path: Path,
                 output_path: Path) -> dict:
    seal, states = verify_v34_seal(manifest_path, sealed_dir)
    ...
    evidence = {
        "schema": "fold-protein-blind-evaluation/v34",
        "status": "completed",
