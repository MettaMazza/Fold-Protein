#!/usr/bin/env python3
"""Verify the V37 seal before opening a comparison target."""
...
def verify_v37_seal(manifest_path: Path, sealed_dir: Path) -> tuple[dict, dict]:
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v37":
        raise RuntimeError("invalid V37 manifest; target access forbidden")
    seal_path = sealed_dir / "seal.json"
    if not seal_path.is_file():
        raise RuntimeError("missing V37 seal; target access forbidden")
    seal = json.loads(seal_path.read_text())
    if (seal.get("schema") != "fold-protein-blind-seal/v37"
            or seal.get("status") != "completed"):
        raise RuntimeError("invalid V37 seal; target access forbidden")
    required = {
    ...
    for field, actual in required.items():
        if seal.get(field) != actual:
            raise RuntimeError(f"V37 seal mismatch: {field}; target access forbidden")
    ...
    active_modes = [mode for mode in states.get("modes", []) if mode != "inactive"]
    actual_census = sorted((active_modes.count("alpha"), active_modes.count("beta")))
    if (states.get("sequence") != sequence
            or len(states.get("states", [])) != len(sequence)
            or states.get("complete_chain_candidates") != 16
            or states.get("partition_span") != 5
            or states.get("partition_unit_count") != (len(sequence) - 1) // 5
            or states.get("expected_unordered_mode_census") != actual_census
            or states.get("qualifying_candidates") != 1
            or len(states.get("census_trace", [])) != 16
            or sum(row["qualifies"] for row in states["census_trace"]) != 1):
        raise RuntimeError("V37 whole-chain partition identity mismatch")
    domain = set(states["closed_domain"].values())
    if any(state not in domain for state in states["states"][:-1]):
        raise RuntimeError("V37 emitted a state outside the engine-closed domain")
    for relative, expected in manifest["source_sha256"].items():
        if seal["source_sha256"].get(relative) != expected or sha256(ROOT / relative) != expected:
            raise RuntimeError(f"V37 source drift: {relative}; target access forbidden")
    return seal, states


def evaluate_v37(manifest_path: Path, sealed_dir: Path, target_path: Path,
                 output_path: Path) -> dict:
    seal, states = verify_v37_seal(manifest_path, sealed_dir)
    ...
    evidence = {
        "schema": "fold-protein-blind-evaluation/v37",
        "status": "completed",
        ...
        "selected_direction": states["selected_direction"],
        "selected_context": states["selected_context"],
        "expected_unordered_mode_census": states["expected_unordered_mode_census"],
        "qualifying_candidates": states["qualifying_candidates"],
    }
