#!/usr/bin/env python3
"""Verify selector-v22 sealing before opening a comparison target."""
...
def verify_v22_seal(manifest_path: Path, sealed_dir: Path) -> tuple[dict, dict]:
    ...
    if (manifest.get("schema") != "fold-protein-blind-selector/v22"
            or seal.get("schema") != "fold-protein-blind-seal/v22"
            or seal.get("status") != "completed"):
        raise RuntimeError("invalid v22 seal; target access forbidden")
    for field, path in {
        ...
        if seal.get(field) != sha256(path):
            raise RuntimeError(f"v22 seal mismatch for {field}")
    ...
    if (states.get("sequence") != sequence
        ...
        raise RuntimeError("v22 input/state identity mismatch")
    if len(states.get("final_relations", {}).get("objectives", [])) != \
            seal.get("constitutional_objectives"):
        raise RuntimeError("v22 objective census mismatch")
    for relative, expected in seal.get("source_sha256", {}).items():
        if (manifest["source_sha256"].get(relative) != expected
            ...
            raise RuntimeError(f"v22 source mismatch for {relative}")
    return seal, states


def evaluate_v22(manifest_path: Path, sealed_dir: Path, target_path: Path,
                 output_path: Path) -> dict:
    seal, states = verify_v22_seal(manifest_path, sealed_dir)
    ...
    result = {
        "schema": "fold-protein-blind-evaluation/v22", "status": "completed",
