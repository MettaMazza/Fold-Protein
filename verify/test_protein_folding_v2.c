...
long long fold_fixed_point_candidate_count();
long long native_state_is_unique_fixed_point();
...
long long expect_equal(long long label, long long got, long long want) {
    ...
    halted = halt_violation(concat((long long)"protein fold equality failed: ", concat(label, concat((long long)" got ", got))));
    }
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long expect_bool(long long label, long long got) {
    ...
    halted = halt_violation(concat((long long)"protein fold closure failed: ", label));
    }
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long _main() {
    ...
    ok = expect_bool((long long)"the One is the unique fixed point across both exact fold branches", native_state_is_unique_fixed_point());
    ...
    ok = expect_bool((long long)"declared 3/4 query reaches the One in two exact folds", descent_reaches_native());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long native_state() {
    ...
    ret_val = fold_value_size(the_one());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long native_state_is_fixed() {
    ...
    one = the_one();
    folded = fold(one);
    ret_val = fold_value_compare(folded, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_FoldValue(folded);
    folded = 0;
    return ret_val;
}
...
long long fold_fixed_point_candidate_count() {
    ...
    long long lower_solution = 0;
    long long one = 0;
    long long upper_solution = 0;
    long long ret_val = 0;
    ...
    one = fold_period(the_one(), b);
    candidates = 0;
    lower_solution = 0;
    if (lower_solution > 0) {
    if (lower_solution <= one) {
    candidates = (candidates + 1);
    }
    }
    upper_solution = (one / (b - one));
    if (upper_solution > 0) {
    if (upper_solution <= one) {
    candidates = (candidates + 1);
    }
    }
    ret_val = candidates;
...
long long native_state_is_unique_fixed_point() {
    ...
    claimed_satisfies = 0;
    if (native_state_is_fixed()) {
    claimed_satisfies = 1;
    }
    alternatives = (fold_fixed_point_candidate_count() - 1);
    guarded = forced_unique((long long)"the One is the unique fixed point across both fold branches", claimed_satisfies, alternatives);
    ret_val = guarded == 1;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long starting_conformation() {
    ...
    ret_val = fold_value_size(supposed_value(colour_count(), (binary_count() * binary_count())));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long descent_after_one_step() {
    ...
    start = supposed_value(colour_count(), (binary_count() * binary_count()));
    ret_val = fold_value_size(fold(start));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    free_struct_FoldValue(start);
    start = 0;
    return ret_val;
}
...
long long descent_after_two_steps() {
    ...
    start = supposed_value(colour_count(), (binary_count() * binary_count()));
    once = fold(start);
    ret_val = fold_value_size(fold(once));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_FoldValue(once);
    once = 0;
    return ret_val;
}
...
long long descent_reaches_native() {
    ...
    native = fold_value_size(the_one());
