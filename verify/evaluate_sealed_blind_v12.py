#!/usr/bin/env python3
"""Verify selector-v12 sealing before opening a comparison target."""
...
def verify_v12_seal(manifest_path: Path, sealed_dir: Path) -> tuple[dict, dict]:
    manifest_path = manifest_path.resolve()
    sealed_dir = sealed_dir.resolve()
    manifest = json.loads(manifest_path.read_text())
    if manifest.get("schema") != "fold-protein-blind-selector/v12":
        raise RuntimeError("invalid v12 manifest; target access forbidden")
    seal_path = sealed_dir / "seal.json"
    if not seal_path.is_file():
        raise RuntimeError("missing v12 seal; target access forbidden")
    seal = json.loads(seal_path.read_text())
    if (seal.get("schema") != "fold-protein-blind-seal/v12"
            or seal.get("status") != "completed"):
        raise RuntimeError("invalid v12 seal; target access forbidden")
    for field, path in {
        ...
        if seal.get(field) != sha256(path):
            raise RuntimeError(f"v12 seal mismatch for {field}")
    ...
    if (states.get("sequence") != sequence
        ...
        raise RuntimeError("v12 input/state identity mismatch")
    if hashlib.sha256(sequence.encode()).hexdigest() != seal.get("sequence_sha256"):
        raise RuntimeError("v12 sequence hash mismatch")
    if len(states.get("orientation_trace", [])) != max(0, len(sequence) - 3):
        raise RuntimeError("v12 orientation trace mismatch")
    for field in ("charge_census", "steric_census",
                  "topology_hydrogen_bond_census"):
        if states.get(field) != seal.get(field):
            raise RuntimeError(f"v12 {field} mismatch")
    if len(states.get("final_relations", {}).get("objectives", [])) != \
            seal.get("constitutional_objectives"):
        raise RuntimeError("v12 objective census mismatch")
    for relative, expected in seal.get("source_sha256", {}).items():
        if (manifest["source_sha256"].get(relative) != expected
            ...
            raise RuntimeError(f"v12 source mismatch for {relative}")
    return seal, states


def evaluate_v12(manifest_path: Path, sealed_dir: Path, target_path: Path,
                 output_path: Path) -> dict:
    seal, states = verify_v12_seal(manifest_path, sealed_dir)
    ...
    result = {
        "schema": "fold-protein-blind-evaluation/v12",
        "status": "completed",
        "run_id": seal["run_id"],
