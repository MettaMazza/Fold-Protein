#!/usr/bin/env python3
"""Verify and evaluate sealed joint-topology selector V26."""
...
def verify(manifest_path, sealed_dir):
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v26" or seal.get("schema") != "fold-protein-blind-seal/v26":
        raise RuntimeError("invalid V26 seal; target access forbidden")
    for field,path in {"protocol_manifest_sha256":manifest_path,"selector_input_sha256":sealed_dir/"selector_input.json","selected_states_sha256":sealed_dir/"selected_states.json","prediction_pdb_sha256":sealed_dir/"prediction.pdb"}.items():
        if seal.get(field) != sha(path): raise RuntimeError(f"V26 seal mismatch: {field}")
    states=json.loads((sealed_dir/"selected_states.json").read_text())
    if len(states["states"]) != len(states["sequence"]) or not all(row["retained_paths"] == 24 for row in states["topology_trace"]):
        raise RuntimeError("V26 paired topology closure mismatch")
    parent=manifest["parent_records"][seal["parent_run_id"]]
    if sha(ROOT/parent["path"]) != parent["sha256"] or sha(ROOT/parent["domain_path"]) != parent["domain_sha256"]:
        raise RuntimeError("V26 registered lineage drift")
    for relative,expected in manifest["source_sha256"].items():
        if sha(ROOT/relative) != expected: raise RuntimeError(f"V26 source drift: {relative}")
    return seal,states
