#!/usr/bin/env python3
"""Verify a selector-v7 seal completely before opening a comparison target."""
...
def verify_v7_seal(manifest_path: Path, sealed_dir: Path) -> tuple[dict, dict]:
    manifest_path = manifest_path.resolve()
    sealed_dir = sealed_dir.resolve()
    manifest = json.loads(manifest_path.read_text())
    if manifest.get("schema") != "fold-protein-blind-selector/v7":
        raise RuntimeError("invalid v7 manifest; target access forbidden")
    seal_path = sealed_dir / "seal.json"
    if not seal_path.is_file():
        raise RuntimeError("missing v7 seal; target access forbidden")
    seal = json.loads(seal_path.read_text())
    if seal.get("schema") != "fold-protein-blind-seal/v7" or \
            seal.get("status") != "completed":
        raise RuntimeError("invalid or incomplete v7 seal; target access forbidden")
    required = {
    ...
    for field, actual in required.items():
        if seal.get(field) != actual:
            raise RuntimeError(f"v7 seal mismatch for {field}; target access forbidden")
    ...
    if states.get("sequence") != sequence or \
        ...
        raise RuntimeError("v7 input/state identity mismatch; target access forbidden")
    if hashlib.sha256(sequence.encode()).hexdigest() != seal.get("sequence_sha256"):
        raise RuntimeError("v7 sequence hash mismatch; target access forbidden")
    if len(states.get("states", [])) != seal.get("path_length") or \
            seal.get("path_length") != len(sequence):
        raise RuntimeError("v7 path length mismatch; target access forbidden")
    ...
    if len(states.get("orientation_trace", [])) != expected_quartets or \
        ...
        raise RuntimeError("v7 orientation trace mismatch; target access forbidden")
    if states.get("orientation_modes") != seal.get("generated_orientation_modes"):
        raise RuntimeError("v7 generated-mode mismatch; target access forbidden")
    if states.get("mode_capacity") != seal.get("mode_capacity") or \
            seal.get("binary_mode_count") * seal.get("mode_capacity") != 24:
        raise RuntimeError("v7 mode-capacity mismatch; target access forbidden")
    for row in states.get("mode_balance_trace", []):
        if set(row) != {"alpha", "beta"}:
            raise RuntimeError("v7 mode balance is incomplete; target access forbidden")
    for relative, expected in seal.get("source_sha256", {}).items():
        if manifest.get("source_sha256", {}).get(relative) != expected or \
            ...
            raise RuntimeError(f"v7 source mismatch for {relative}; target access forbidden")
    return seal, states


def evaluate_v7(manifest_path: Path, sealed_dir: Path, target_path: Path,
                output_path: Path) -> dict:
    seal, states = verify_v7_seal(manifest_path, sealed_dir)
    ...
    evidence = {
        "schema": "fold-protein-blind-evaluation/v7",
        "status": "completed",
        "run_id": seal["run_id"],
