#!/usr/bin/env python3
"""Verify selector-v20 sealing before opening a comparison target."""
...
def verify_v20_seal(manifest_path: Path, sealed_dir: Path) -> tuple[dict, dict]:
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v20":
        raise RuntimeError("invalid v20 manifest; target access forbidden")
    seal_path = sealed_dir / "seal.json"
    if not seal_path.is_file():
        raise RuntimeError("missing v20 seal; target access forbidden")
    seal = json.loads(seal_path.read_text())
    if (seal.get("schema") != "fold-protein-blind-seal/v20"
            or seal.get("status") != "completed"):
        raise RuntimeError("invalid v20 seal; target access forbidden")
    for field, path in {
        ...
        if seal.get(field) != sha256(path):
            raise RuntimeError(f"v20 seal mismatch for {field}")
    ...
    if (states.get("sequence") != sequence
        ...
        raise RuntimeError("v20 input/state identity mismatch")
    if hashlib.sha256(sequence.encode()).hexdigest() != seal.get("sequence_sha256"):
        raise RuntimeError("v20 sequence hash mismatch")
    if (states.get("hard_exclusion_strata") != seal.get("hard_exclusion_strata")
        ...
        raise RuntimeError("v20 global graph hierarchy mismatch")
    if len(states.get("orientation_trace", [])) != max(0, len(sequence) - 3):
        raise RuntimeError("v20 orientation trace mismatch")
    for field in (
        ...
        if states.get(field) != seal.get(field):
            raise RuntimeError(f"v20 {field} mismatch")
    if len(states.get("final_relations", {}).get("objectives", [])) != \
            seal.get("constitutional_objectives"):
        raise RuntimeError("v20 objective census mismatch")
    for relative, expected in seal.get("source_sha256", {}).items():
        if (manifest["source_sha256"].get(relative) != expected
            ...
            raise RuntimeError(f"v20 source mismatch for {relative}")
    return seal, states


def evaluate_v20(manifest_path: Path, sealed_dir: Path, target_path: Path,
                 output_path: Path) -> dict:
    seal, states = verify_v20_seal(manifest_path, sealed_dir)
    ...
    result = {
        "schema": "fold-protein-blind-evaluation/v20",
        "status": "completed",
