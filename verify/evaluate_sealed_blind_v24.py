#!/usr/bin/env python3
"""Verify and evaluate sealed coherent-segment selector V24."""
...
def verify(manifest_path, sealed_dir):
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v24" or seal.get("schema") != "fold-protein-blind-seal/v24":
        raise RuntimeError("invalid V24 seal; target access forbidden")
    for field,path in {"protocol_manifest_sha256":manifest_path,"selector_input_sha256":sealed_dir/"selector_input.json","selected_states_sha256":sealed_dir/"selected_states.json","prediction_pdb_sha256":sealed_dir/"prediction.pdb"}.items():
        if seal.get(field) != sha(path): raise RuntimeError(f"V24 seal mismatch: {field}")
    states=json.loads((sealed_dir/"selected_states.json").read_text())
    if len(states["states"]) != len(states["sequence"]) or any(row["retained_tuple_count"] != 24 for row in states["segment_generation_trace"]):
        raise RuntimeError("V24 coherent segment closure mismatch")
    parent=manifest["parent_records"][seal["parent_run_id"]]
    if sha(ROOT/parent["path"]) != parent["sha256"]: raise RuntimeError("V24 parent drift")
    for relative,expected in manifest["source_sha256"].items():
        if sha(ROOT/relative) != expected: raise RuntimeError(f"V24 source drift: {relative}")
    return seal,states
