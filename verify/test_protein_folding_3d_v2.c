...
long long protein_angle_lattice_count();
long long alpha_phi_lattice_index();
long long alpha_psi_lattice_index();
long long beta_phi_lattice_index();
long long beta_psi_lattice_index();
long long fold_angle_lattice_index(long long);
long long fold_angle_lattice_index_to_colour_depth(long long);
long long protein_angle_absolute_index(long long);
long long protein_angle_index_sign(long long);
long long protein_angle_transient_base_index();
long long alpha_phi_form_candidate_count();
long long beta_phi_form_candidate_count();
long long alpha_psi_form_candidate_count();
long long beta_psi_form_candidate_count();
long long canonical_angle_forms_are_unique();
long long canonical_angle_coordinates_match_forced_indices();
long long enforce_canonical_angle_form_uniqueness();
...
long long protein_orientation_window();
long long protein_orientation_overlap();
long long protein_orientation_one_count();
long long protein_orientation_advance();
long long protein_orientation_overlap_candidates();
long long protein_orientation_overlap_is_unique();
long long protein_orientation_window_closes();
long long protein_orientation_overlap_closes();
long long protein_orientation_advance_is_the_one();
long long protein_homochiral_phi_sign();
long long protein_opposite_orientation_sign();
long long protein_homochiral_sign_candidate_count();
long long protein_opposite_sign_candidate_count();
long long enforce_protein_orientation_signs();
...
long long expect_equal(long long label, long long got, long long want) {
    ...
    halted = halt_violation(concat((long long)"protein angle equality failed: ", concat(label, concat((long long)" got ", got))));
    }
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long expect_bool(long long label, long long got) {
    ...
    halted = halt_violation(concat((long long)"protein angle check failed: ", label));
    }
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long _main() {
    ...
    ok = expect_bool((long long)"angle lattice is assembled from the binary and colour counts", protein_angle_lattice_count() == (whole_power(binary_count(), colour_count()) * colour_count()));
    ok = expect_bool((long long)"homochiral signed-chart orientation has one complete candidate", protein_homochiral_sign_candidate_count() == 1);
    ok = expect_bool((long long)"opposite signed-chart orientation has one complete candidate", protein_opposite_sign_candidate_count() == 1);
    ok = expect_bool((long long)"alpha phi has one form in the complete signed lattice", alpha_phi_form_candidate_count() == 1);
    ok = expect_bool((long long)"alpha psi has one form in the complete signed lattice", alpha_psi_form_candidate_count() == 1);
    ok = expect_bool((long long)"beta phi has one form in the complete signed lattice", beta_phi_form_candidate_count() == 1);
    ok = expect_bool((long long)"beta psi has one form in the complete signed lattice", beta_psi_form_candidate_count() == 1);
    ok = expect_bool((long long)"canonical coordinates match the independently assembled indices", canonical_angle_coordinates_match_forced_indices());
    ok = expect_bool((long long)"canonical angle forms pass the engine admission guard", enforce_canonical_angle_form_uniqueness() == 1);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long alpha_phi_coord() {
    ...
    f = fraction_from_ratio(alpha_phi_lattice_index(), protein_angle_lattice_count());
    ((EpStruct_Fraction*)(f))->numerator_sign = protein_homochiral_phi_sign();
    ep_gc_write_barrier((void*)(f), protein_homochiral_phi_sign());
    ret_val = f;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long alpha_psi_coord() {
    ...
    f = fraction_from_ratio(alpha_psi_lattice_index(), protein_angle_lattice_count());
    ((EpStruct_Fraction*)(f))->numerator_sign = protein_homochiral_phi_sign();
    ep_gc_write_barrier((void*)(f), protein_homochiral_phi_sign());
    ret_val = f;
    goto L_cleanup;
...
long long beta_phi_coord() {
    ...
    f = fraction_from_ratio(beta_phi_lattice_index(), protein_angle_lattice_count());
    ((EpStruct_Fraction*)(f))->numerator_sign = protein_homochiral_phi_sign();
    ep_gc_write_barrier((void*)(f), protein_homochiral_phi_sign());
    ret_val = f;
    goto L_cleanup;
...
long long beta_psi_coord() {
    ...
    f = fraction_from_ratio(beta_psi_lattice_index(), protein_angle_lattice_count());
    ((EpStruct_Fraction*)(f))->numerator_sign = protein_opposite_orientation_sign();
    ep_gc_write_barrier((void*)(f), protein_opposite_orientation_sign());
    ret_val = f;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long protein_angle_lattice_count() {
    ...
    ret_val = (whole_power(binary_count(), colour_count()) * smallest_fold_period_above(binary_count()));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long alpha_phi_lattice_index() {
    long long generator_route = 0;
    long long lattice_route = 0;
    long long ret_val = 0;

    ep_gc_push_root(&generator_route);
    ep_gc_push_root(&lattice_route);

    ep_gc_maybe_collect();

    lattice_route = ((whole_power(binary_count(), colour_count()) * smallest_fold_period_above(binary_count())) / (smallest_fold_period_above(1) * smallest_fold_period_above(binary_count())));
    generator_route = (smallest_fold_period_above(1) * smallest_fold_period_above(1));
    ret_val = forced_to_be((long long)"alpha phi lattice index", lattice_route, generator_route);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long alpha_psi_lattice_index() {
    ...
    lattice_route = ((whole_power(binary_count(), colour_count()) * smallest_fold_period_above(binary_count())) / whole_power(binary_count(), colour_count()));
    generator_route = smallest_fold_period_above(binary_count());
    ret_val = forced_to_be((long long)"alpha psi lattice index", lattice_route, generator_route);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long beta_phi_lattice_index() {
    ...
    lattice_route = ((whole_power(binary_count(), colour_count()) * smallest_fold_period_above(binary_count())) / smallest_fold_period_above(binary_count()));
    generator_route = whole_power(binary_count(), colour_count());
    ret_val = forced_to_be((long long)"beta phi lattice index", lattice_route, generator_route);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long beta_psi_lattice_index() {
    long long generator_route = 0;
    long long window_route = 0;
    long long ret_val = 0;

    ep_gc_push_root(&generator_route);
    ep_gc_push_root(&window_route);

    ep_gc_maybe_collect();

    window_route = (smallest_fold_period_above(binary_count()) * alpha_psi_lattice_index());
    generator_route = (smallest_fold_period_above(binary_count()) * smallest_fold_period_above(binary_count()));
    ret_val = forced_to_be((long long)"beta psi lattice index", window_route, generator_route);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long fold_angle_lattice_index(long long index) {
    long long folded = 0;
    long long lattice = 0;
    long long ret_val = 0;

    lattice = (whole_power(binary_count(), colour_count()) * smallest_fold_period_above(binary_count()));
    folded = ((smallest_fold_period_above(1) * index) % lattice);
    if (folded == 0) {
    ret_val = lattice;
    goto L_cleanup;
    }
...
long long fold_angle_lattice_index_to_colour_depth(long long index) {
    long long current = 0;
    long long step = 0;
    long long ret_val = 0;
    ...
    current = index;
    step = 0;
    while (step < smallest_fold_period_above(binary_count())) {
    current = fold_angle_lattice_index(current);
    step = (step + 1);
    }
    ret_val = current;
    goto L_cleanup;
...
long long protein_angle_absolute_index(long long index) {
    long long ret_val = 0;
...
long long protein_angle_index_sign(long long index) {
    ...
    magnitude = protein_angle_absolute_index(index);
    if (magnitude == 0) {
    ret_val = 0;
    goto L_cleanup;
    }
    ret_val = (index / magnitude);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long alpha_phi_form_candidate_count() {
    ...
    lattice = (whole_power(binary_count(), colour_count()) * smallest_fold_period_above(binary_count()));
    target = (lattice / smallest_fold_period_above(binary_count()));
    half = (lattice / smallest_fold_period_above(1));
    matching = 0;
    candidate = (0 - lattice);
    while (candidate <= lattice) {
    if (protein_angle_index_sign(candidate) == protein_homochiral_phi_sign()) {
    magnitude = protein_angle_absolute_index(candidate);
    if (magnitude < half) {
...
long long beta_phi_form_candidate_count() {
    ...
    lattice = (whole_power(binary_count(), colour_count()) * smallest_fold_period_above(binary_count()));
    target = ((smallest_fold_period_above(1) * lattice) / smallest_fold_period_above(binary_count()));
    half = (lattice / smallest_fold_period_above(1));
    matching = 0;
    candidate = (0 - lattice);
    while (candidate <= lattice) {
...
long long alpha_psi_form_candidate_count() {
    ...
    lattice = (whole_power(binary_count(), colour_count()) * smallest_fold_period_above(binary_count()));
    required = (protein_orientation_advance() * protein_angle_transient_base_index());
    matching = 0;
    candidate = (0 - lattice);
    while (candidate <= lattice) {
    if (protein_angle_index_sign(candidate) == protein_homochiral_phi_sign()) {
...
long long beta_psi_form_candidate_count() {
    ...
    lattice = (whole_power(binary_count(), colour_count()) * smallest_fold_period_above(binary_count()));
    required = (smallest_fold_period_above(binary_count()) * protein_angle_transient_base_index());
    matching = 0;
    candidate = (0 - lattice);
    while (candidate <= lattice) {
...
long long canonical_angle_coordinates_match_forced_indices() {
    long long alpha_phi_expected = 0;
    long long alpha_psi_expected = 0;
    long long beta_phi_expected = 0;
    long long beta_psi_expected = 0;
    long long ret_val = 0;

    ep_gc_push_root(&alpha_phi_expected);
    ep_gc_push_root(&alpha_psi_expected);
    ep_gc_push_root(&beta_phi_expected);
    ep_gc_push_root(&beta_psi_expected);

    ep_gc_maybe_collect();

    alpha_phi_expected = fraction_from_ratio(alpha_phi_lattice_index(), protein_angle_lattice_count());
    ((EpStruct_Fraction*)(alpha_phi_expected))->numerator_sign = protein_homochiral_phi_sign();
    ep_gc_write_barrier((void*)(alpha_phi_expected), protein_homochiral_phi_sign());
    if (fraction_compare(alpha_phi_coord(), alpha_phi_expected) == 0) {
    alpha_psi_expected = fraction_from_ratio(alpha_psi_lattice_index(), protein_angle_lattice_count());
    ((EpStruct_Fraction*)(alpha_psi_expected))->numerator_sign = protein_homochiral_phi_sign();
    ep_gc_write_barrier((void*)(alpha_psi_expected), protein_homochiral_phi_sign());
    if (fraction_compare(alpha_psi_coord(), alpha_psi_expected) == 0) {
    beta_phi_expected = fraction_from_ratio(beta_phi_lattice_index(), protein_angle_lattice_count());
    ((EpStruct_Fraction*)(beta_phi_expected))->numerator_sign = protein_homochiral_phi_sign();
    ep_gc_write_barrier((void*)(beta_phi_expected), protein_homochiral_phi_sign());
    if (fraction_compare(beta_phi_coord(), beta_phi_expected) == 0) {
    beta_psi_expected = fraction_from_ratio(beta_psi_lattice_index(), protein_angle_lattice_count());
    ((EpStruct_Fraction*)(beta_psi_expected))->numerator_sign = protein_opposite_orientation_sign();
    ep_gc_write_barrier((void*)(beta_psi_expected), protein_opposite_orientation_sign());
    if (fraction_compare(beta_psi_coord(), beta_psi_expected) == 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
    }
...
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(alpha_phi_expected);
    alpha_phi_expected = 0;
    free_struct_Fraction(alpha_psi_expected);
    alpha_psi_expected = 0;
    free_struct_Fraction(beta_phi_expected);
    beta_phi_expected = 0;
    free_struct_Fraction(beta_psi_expected);
    beta_psi_expected = 0;
    return ret_val;
}

long long enforce_canonical_angle_form_uniqueness() {
    long long alpha_phi_guard = 0;
    long long alpha_psi_guard = 0;
    long long beta_phi_guard = 0;
    long long beta_psi_guard = 0;
    long long coordinates_match = 0;
    long long sign_guard = 0;
    long long ret_val = 0;

    ep_gc_push_root(&coordinates_match);

    ep_gc_maybe_collect();

    sign_guard = enforce_protein_orientation_signs();
    coordinates_match = 0;
    if (canonical_angle_coordinates_match_forced_indices()) {
    coordinates_match = 1;
    }
    alpha_phi_guard = forced_unique((long long)"right-handed alpha phi coordinate form", coordinates_match, (alpha_phi_form_candidate_count() - 1));
    alpha_psi_guard = forced_unique((long long)"right-handed alpha psi coordinate form", coordinates_match, (alpha_psi_form_candidate_count() - 1));
    beta_phi_guard = forced_unique((long long)"beta phi coordinate form", coordinates_match, (beta_phi_form_candidate_count() - 1));
    beta_psi_guard = forced_unique((long long)"beta psi coordinate form", coordinates_match, (beta_psi_form_candidate_count() - 1));
    if (sign_guard == 1) {
    ...
    ret_val = halt_violation((long long)"canonical Protein angle form admission did not close");
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long protein_orientation_one_count() {
    ...
    ret_val = fold_period(the_one(), binary_count());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long protein_orientation_advance() {
    long long from_overlap = 0;
    long long from_the_one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&from_overlap);
    ep_gc_push_root(&from_the_one);

    ep_gc_maybe_collect();

    from_overlap = (smallest_fold_period_above(binary_count()) - smallest_fold_period_above(1));
    from_the_one = fold_period(the_one(), binary_count());
    ret_val = forced_to_be((long long)"protein colour-window advance", from_overlap, from_the_one);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long protein_orientation_overlap_candidates() {
    ...
    window = smallest_fold_period_above(binary_count());
    one = fold_period(the_one(), binary_count());
    matching = 0;
    candidate = 0;
...
long long protein_orientation_overlap_is_unique() {
    ...
    long long claimed_satisfies = 0;
    long long closed_overlap = 0;
    long long guarded = 0;
    long long independently_forced = 0;
    long long one = 0;
    long long overlap = 0;
    long long window = 0;
    long long ret_val = 0;
    ...
    ep_gc_push_root(&claimed_satisfies);
    ep_gc_push_root(&independently_forced);
    ...
    window = smallest_fold_period_above(binary_count());
    overlap = smallest_fold_period_above(1);
    one = fold_period(the_one(), binary_count());
    claimed_satisfies = 0;
    if ((window - overlap) == one) {
    claimed_satisfies = 1;
    }
    alternatives = (protein_orientation_overlap_candidates() - 1);
    guarded = forced_unique((long long)"binary overlap is the unique colour-window overlap with One advance", claimed_satisfies, alternatives);
    independently_forced = (window - one);
    closed_overlap = forced_to_be((long long)"binary overlap equals colour window less the One", overlap, independently_forced);
    if (guarded == 1) {
...
long long protein_orientation_overlap_closes() {
    ...
    ret_val = protein_orientation_overlap_is_unique();
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long protein_orientation_advance_is_the_one() {
    ...
    ret_val = protein_orientation_advance() == fold_period(the_one(), binary_count());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long protein_homochiral_phi_sign() {
    long long from_binary_mirror = 0;
    long long from_chart_orientation = 0;
    long long one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&from_binary_mirror);
    ep_gc_push_root(&from_chart_orientation);

    ep_gc_maybe_collect();

    one = fold_period(the_one(), binary_count());
    from_chart_orientation = (0 - one);
    from_binary_mirror = ((0 - smallest_fold_period_above(1)) + one);
    ret_val = forced_to_be((long long)"canonical signed-chart representative of the SFT homochiral hand", from_chart_orientation, from_binary_mirror);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long protein_opposite_orientation_sign() {
    long long from_antipode = 0;
    long long from_the_one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&from_antipode);
    ep_gc_push_root(&from_the_one);

    ep_gc_maybe_collect();

    from_antipode = (0 - protein_homochiral_phi_sign());
    from_the_one = fold_period(the_one(), binary_count());
    ret_val = forced_to_be((long long)"opposite signed-chart orientation", from_antipode, from_the_one);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long protein_homochiral_sign_candidate_count() {
    ...
    one = fold_period(the_one(), binary_count());
    required = protein_homochiral_phi_sign();
    matching = 0;
    candidate = (0 - one);
    while (candidate <= one) {
    if (candidate != 0) {
...
long long protein_opposite_sign_candidate_count() {
    ...
    one = fold_period(the_one(), binary_count());
    required = protein_opposite_orientation_sign();
    matching = 0;
    candidate = (0 - one);
    while (candidate <= one) {
    if (candidate != 0) {
...
long long enforce_protein_orientation_signs() {
    long long homochiral_guard = 0;
    long long opposite_guard = 0;
    long long ret_val = 0;

    homochiral_guard = forced_unique((long long)"Protein homochiral signed-chart representative", 1, (protein_homochiral_sign_candidate_count() - 1));
    opposite_guard = forced_unique((long long)"Protein opposite signed-chart representative", 1, (protein_opposite_sign_candidate_count() - 1));
    ...
    ret_val = halt_violation((long long)"Protein orientation sign admission did not close");
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
