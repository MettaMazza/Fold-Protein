#!/usr/bin/env python3
"""Selector V33: exact minimax cross-topology consensus.

The maximum counted distance to any complete degree-family frontier is the
outer consensus stratum.  Only candidates attaining the minimum such maximum
enter symmetric family-distance balance, and only that minimax frontier enters
the established whole-chain constitution.  This is an exact constitutional
order, not a scalar blend, reward, quota, lock or fitted parameter.
"""
...
from tools.blind_24_lattice_selector_v32 import _consensus_relation, _state_distance
...
def select_state_path_v33(sequence: str, v25_states, v26_1_states,
    ...
    for name, states in zip(names, branches):
        if (len(states) != len(sequence) or states[-1] != CANONICAL_STATE
            ...
            raise ValueError(f"V33 requires a valid sealed {name.upper()} branch")
    ...
    consensus_raw = {states: _consensus_relation(states, topology_frontiers)
                     for states in all_candidates}
    minimax_value = min(row["worst_family_distance"]
                        for row in consensus_raw.values())
    minimax_candidates = [states for states, row in consensus_raw.items()
                          if row["worst_family_distance"] == minimax_value]
    consensus_rows = [((0,), tuple(
        consensus_raw[states]["family_minimum_state_distances"]), states)
        for states in minimax_candidates]
    consensus_frontier = select_balanced_hierarchy(
        consensus_rows, min(FRONTIER_CAPACITY, len(consensus_rows)))
    ...
    final_frontier = select_balanced_hierarchy(
        rich_rows, min(FRONTIER_CAPACITY, len(rich_rows)))
    ...
    return {
        ...
        **{name + "_departures": _state_distance(states, branch)
        ...
        "minimax_consensus": {
            ...
            "minimax_worst_family_distance": minimax_value,
            "minimax_candidate_count": len(minimax_candidates),
