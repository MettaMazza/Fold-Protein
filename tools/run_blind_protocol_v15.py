#!/usr/bin/env python3
"""Run and immutably seal selector v15 before target comparison."""
...
from tools.blind_24_lattice_selector_v15 import (
    ...
    TOPOLOGY_HYDROGEN_BOND_CENSUS, select_state_path_v15)
...
def run_protocol_v15(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v15":
        raise ValueError("unsupported selector-v15 manifest")
    ...
    if set(selector_input) != {"run_id", "sequence"}:
        raise ValueError("selector-v15 input must contain run_id and sequence")
    expected = {
        "beam_width": 24,
    ...
    if manifest.get("selector_config") != expected:
        raise RuntimeError("selector-v15 configuration is not the forced census")

    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(
        prefix="fold-protein-v15-sealed-", dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v15(selector_input["sequence"])
        state_record = {
            "schema": "fold-protein-selected-states/v15",
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v15",
            "status": "completed",
            "result_type": "cumulative development benchmark",
            "official_run": False,
            "execution": (
                "sequence-only symmetric ordinal v13 assembly with "
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v15(
