#!/usr/bin/env python3
"""Run and seal the complete V45 boundary-axis fixed-point frontier."""
...
from tools.blind_24_lattice_selector_v45 import select_state_frontier_v45
...
def run_protocol_v45(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v45":
        raise ValueError("unsupported V45 manifest")
    if set(selector_input) != {"run_id", "sequence"}:
        raise ValueError("V45 input must contain exactly run_id and sequence")
    ...
    if manifest.get("parent_v42") != parent_v42:
        raise RuntimeError("V45 V42 parent drifted")
    parent = json.loads((V42_DIR / "frontier.json").read_text())

    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(
        prefix="fold-protein-v45-sealed-", dir=output_dir.parent
    ))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_frontier_v45(selector_input["sequence"], parent)
        rows, pdb_hashes = [], {}
        ...
        record = {
            "schema": "fold-protein-selected-frontier/v45",
            "status": "completed",
            ...
            "fixed_point_trace_count": result["fixed_point_trace_count"],
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v45",
            "status": "completed",
            "result_type": "cumulative development benchmark",
            "official_run": False,
            "execution": "all connected parents crossed with every boundary-axis coordinate fixed-point order",
            "run_id": selector_input["run_id"],
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v45(
