#!/usr/bin/env python3
"""Run and immutably seal the binary-overlap selector-v4 route."""
...
try:
    from tools.blind_24_lattice_selector_v4 import (
        OVERLAP_CONSTITUTION, forced_beam_width, select_state_path_v4)
    from tools.protein_backbone_geometry_v1 import write_pdb
except ImportError:
    from blind_24_lattice_selector_v4 import (
        OVERLAP_CONSTITUTION, forced_beam_width, select_state_path_v4)
...
def _load_input(path: Path) -> tuple[dict, bytes]:
    ...
    if set(value) != {"run_id", "sequence"}:
        raise ValueError("selector-v4 input must contain exactly run_id and sequence")
...
def run_protocol_v4(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v4":
        raise ValueError("unsupported selector-v4 manifest")
    _check_source_hashes(manifest)
    selector_input, input_raw = _load_input(input_path)
    expected_config = {
        "beam_width": forced_beam_width(),
        "window_residues": OVERLAP_CONSTITUTION["window_residues"],
        "overlap_residues": OVERLAP_CONSTITUTION["overlap_residues"],
    }
    if manifest.get("selector_config") != expected_config:
        raise RuntimeError("selector-v4 configuration is not the forced census")

    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(prefix="fold-protein-v4-sealed-",
                                  dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v4(selector_input["sequence"])
        state_record = {
            "schema": "fold-protein-selected-states/v4",
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v4",
            "status": "completed",
            "execution": "sequence-only binary-overlap selector output",
            "run_id": selector_input["run_id"],
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v4(
