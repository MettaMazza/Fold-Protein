#!/usr/bin/env python3
"""Run and immutably seal coherent-segment selector V24."""
...
def _registered_parent(manifest, parent_run_id, sequence):
    ...
    if not isinstance(binding, dict) or set(binding) != {"path", "sha256"}:
        raise RuntimeError("V24 parent record is not registered")
    path = ROOT / binding["path"]
    if not path.is_file() or sha256_file(path) != binding["sha256"]:
        raise RuntimeError("V24 registered V23.2 parent drifted")
    record = json.loads(path.read_text())
    if (record.get("schema") != "fold-protein-selected-states/v23.2"
            or record.get("run_id") != parent_run_id
            or record.get("sequence") != sequence):
        raise RuntimeError("V24 parent identity did not close")
    return record, binding


def run_protocol_v24(manifest_path: Path, input_path: Path, output_dir: Path):
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v24":
        raise ValueError("unsupported selector-v24 manifest")
    for relative, expected in manifest["source_sha256"].items():
        if sha256_file(ROOT / relative) != expected:
            raise RuntimeError(f"protocol source drift: {relative}")
    if set(selector_input) != {"run_id", "sequence", "parent_run_id"}:
        raise ValueError("V24 input must contain run_id, sequence, parent_run_id")
    ...
    expected_config = {
        ...
        "segment_residues": 4, "segment_frontier_capacity": 24,
        "frontier_capacity": 24,
        "segment_construction": "V19-wrapped V13 local tuple frontier",
    ...
    if manifest.get("selector_config") != expected_config:
        raise RuntimeError("selector-v24 configuration drift")
    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(prefix="fold-protein-v24-sealed-",
                                  dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v24(
            sequence, parent["seed_states"], parent["domain_trace"])
        record = {
            "schema": "fold-protein-selected-states/v24", "status": "completed",
            ...
            "segment_generation_trace": result["segment_generation_trace"],
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v24", "status": "completed",
            "result_type": "cumulative development benchmark", "official_run": False,
            "execution": "coherent local segment frontiers with bidirectional global assembly",
            ...
            "segment_frontier_capacity": FRONTIER_CAPACITY,
...
def main():
    ...
    a = p.parse_args(); print(json.dumps(run_protocol_v24(
