#!/usr/bin/env python3
"""Run and immutably seal selector v14 before target comparison."""
...
from tools.blind_24_lattice_selector_v14 import (
    ...
    HYDROGEN_BOND_CONTINUITY_CENSUS, STERIC_CENSUS,
    TOPOLOGY_HYDROGEN_BOND_CENSUS, select_state_path_v14)
...
def run_protocol_v14(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v14":
        raise ValueError("unsupported selector-v14 manifest")
    ...
    if set(selector_input) != {"run_id", "sequence"}:
        raise ValueError("selector-v14 input must contain run_id and sequence")
    expected = {
        "beam_width": 24,
        ...
        "continuity_successor_distance": 1,
        "continuity_classes": 2,
    }
    if manifest.get("selector_config") != expected:
        raise RuntimeError("selector-v14 configuration is not the forced census")

    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(
        prefix="fold-protein-v14-sealed-", dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v14(selector_input["sequence"])
        state_record = {
            "schema": "fold-protein-selected-states/v14",
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v14",
            "status": "completed",
            "result_type": "cumulative development benchmark",
            "official_run": False,
            "execution": (
                "sequence-only symmetric ordinal selector retaining v13 "
                "assembly and adding exact alpha/inter-strand continuity"),
            "run_id": selector_input["run_id"],
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v14(
