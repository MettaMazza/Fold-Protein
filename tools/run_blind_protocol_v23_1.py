#!/usr/bin/env python3
"""Run and immutably seal the parent-continuous V23.1 correction."""
...
def run_protocol_v23_1(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v23.1":
        raise ValueError("unsupported selector-v23.1 manifest")
    ...
    if set(selector_input) != {"run_id", "sequence", "seed_run_id"}:
        raise ValueError("selector-v23.1 input must contain run_id, sequence, seed_run_id")
    ...
    expected = {
        "lattice_axis": 24, "complete_domain_states": DOMAIN_STATE_COUNT,
        "domain_capacity": DOMAIN_CAPACITY, "frontier_capacity": FRONTIER_CAPACITY,
        "cache_capacity": CACHE_CAPACITY, "segment_residues": SEGMENT_RESIDUES,
        "assembly_directions": ["forward", "reverse"],
        "parent_continuity_relation": "exact state departures from registered sealed V13 path",
        "parent_departure_limit": None, "parent_departure_weight": None,
    ...
    if manifest.get("selector_config") != expected:
        raise RuntimeError("selector-v23.1 configuration is not the forced census")
    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(prefix="fold-protein-v23-1-sealed-",
                                  dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v23_1(sequence, seed_record["states"])
        state_record = {
            "schema": "fold-protein-selected-states/v23.1",
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v23.1", "status": "completed",
            "result_type": "cumulative development benchmark", "official_run": False,
            "execution": "V23 global assembly with weight-free parent continuity",
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v23_1(
