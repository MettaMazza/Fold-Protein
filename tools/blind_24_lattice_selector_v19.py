#!/usr/bin/env python3
"""Selector v19: preserve V13 locally; apply graph exclusions globally.

The explicit v17 side-chain graph is a complete-prefix assembly relation, not
a four-residue local pruning rule. V19 therefore preserves v13 local candidate
eligibility byte-for-byte and applies the hierarchical backbone/side-chain hard
vector only at complete-prefix selection. All fifteen v13 objectives remain.
"""
...
from tools.blind_24_lattice_selector_v18 import (
    ...
    generated_prefix_relations_v18)
...
def generated_local_relations_v19(sequence: str, state_path: list[int]):
    local = generated_local_relations_v13(sequence, state_path)
    return (local[0], (int(local[1]), 0)) + tuple(local[2:])


def generated_prefix_relations_v19(sequence: str, state_path: list[int]):
    return generated_prefix_relations_v18(sequence, state_path)
...
def select_state_path_v19(sequence: str) -> dict:
    ...
    for index in range(len(sequence) - 1):
        ...
        for mode in MODE_NAMES:
            ...
            for _, _, candidate in eligible:
                complete = generated_prefix_relations_v19(sequence, list(candidate))
    ...
    for path in beam:
        ...
        complete = generated_prefix_relations_v19(sequence, list(path))
