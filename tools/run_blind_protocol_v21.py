#!/usr/bin/env python3
"""Run and immutably seal selector v21 before target comparison."""
...
from tools.blind_24_lattice_selector_v21 import (
    CHARGE_CENSUS, CONSTITUTIONAL_OBJECTIVES, CONTACT_SHELL,
    HARD_EXCLUSION_STRATA, HYDROGEN_BOND_CENSUS,
    SIDECHAIN_GRAPH_SPATIAL_CENSUS, STERIC_CENSUS,
    TOPOLOGY_HYDROGEN_BOND_CENSUS, select_state_path_v21)
...
def run_protocol_v21(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v21":
        raise ValueError("unsupported selector-v21 manifest")
    ...
    if set(selector_input) != {"run_id", "sequence"}:
        raise ValueError("selector-v21 input must contain run_id and sequence")
    expected = {
        "beam_width": 24,
        ...
        "contact_objective": "negative_explicit_atom_contact_count",
        "spatial_non_neighbour_separation": 2,
    }
    if manifest.get("selector_config") != expected:
        raise RuntimeError("selector-v21 configuration is not the forced census")
    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(
        prefix="fold-protein-v21-sealed-", dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v21(selector_input["sequence"])
        state_record = {
            "schema": "fold-protein-selected-states/v21",
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v21",
            "status": "completed",
            "result_type": "cumulative development benchmark",
            "official_run": False,
            "execution": "V13-local explicit graph contact topology in complete-prefix assembly",
            "run_id": selector_input["run_id"],
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v21(
