#!/usr/bin/env python3
"""Selector v13: retain local assembly while separating global topology.

V13 carries v11's undifferentiated unit-capacity hydrogen-bond relation beside
v12's alpha/inter-strand relations.  All three enter the same permutation-
invariant ordinal balance; no mixing weight, cutoff, reward, target, template,
or learned quantity is introduced.
"""
...
try:
    ...
    from tools.blind_24_lattice_selector_v12 import (
        generated_local_relations_v12, generated_prefix_relations_v12)
    ...
    from blind_24_lattice_selector_v9 import (
        CHARGE_CENSUS, ORIENTATION_QUARTET, STERIC_CENSUS)
    from blind_24_lattice_selector_v12 import (
        generated_local_relations_v12, generated_prefix_relations_v12)
...
def generated_prefix_relations_v13(sequence: str, state_path: list[int]):
    topology = generated_prefix_relations_v12(sequence, state_path)
    ...
    return tuple(topology) + (retained,)


def generated_local_relations_v13(sequence: str, state_path: list[int]):
    topology = generated_local_relations_v12(sequence, state_path)
    prefix_length = len(state_path) + 1
    if prefix_length < ORIENTATION_QUARTET:
        return tuple(topology) + (0.0,)
    atoms = build_lookahead_prefix(sequence, state_path)
...
def select_state_path_v13(sequence: str) -> dict:
    ...
    for index in range(len(sequence) - 1):
        ...
        for path in beam:
            for state in active_candidates(index, len(sequence)):
                ...
                local = generated_local_relations_v13(sequence, list(candidate))
        ...
        for mode in MODE_NAMES:
            ...
            for _, _, candidate in eligible:
                complete = generated_prefix_relations_v13(
    ...
    final_raw = {}
    for path in beam:
        local = generated_local_relations_v13(sequence, list(path))
        complete = generated_prefix_relations_v13(sequence, list(path))
