#!/usr/bin/env python3
"""Selector v15: V13 assembly with spatial side-chain hard exclusion.

V15 preserves the fifteen v13 objectives exactly once. It extends only the
absolute hard-exclusion stratum with the target-incapable spatial side-chain
encounter census generated from L-chiral backbone frames and exact heavy-atom
graph counts. No soft objective, weight, cutoff, target, reward, rotamer,
empirical radius, or learned quantity is added.
"""
...
def generated_prefix_relations_v15(sequence: str, state_path: list[int]):
...
def generated_local_relations_v15(sequence: str, state_path: list[int]):
...
def select_state_path_v15(sequence: str) -> dict:
    ...
    for index in range(len(sequence) - 1):
        ...
        for path in beam:
            for state in active_candidates(index, len(sequence)):
                ...
                local = generated_local_relations_v15(
        ...
        for mode in MODE_NAMES:
            ...
            for _, _, candidate in eligible:
                complete = generated_prefix_relations_v15(
                    sequence, list(candidate))
    ...
    final_raw = {}
    for path in beam:
        local = generated_local_relations_v15(sequence, list(path))
        complete = generated_prefix_relations_v15(sequence, list(path))
