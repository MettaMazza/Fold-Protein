#!/usr/bin/env python3
"""Run and immutably seal locally closed selector V23.2."""
...
from tools.run_blind_protocol_v23 import ROOT, _registered_seed


def run_protocol_v23_2(manifest_path: Path, input_path: Path, output_dir: Path):
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v23.2":
        raise ValueError("unsupported selector-v23.2 manifest")
    for relative, expected in manifest["source_sha256"].items():
        if sha256_file(ROOT / relative) != expected:
            raise RuntimeError(f"protocol source drift: {relative}")
    if set(selector_input) != {"run_id", "sequence", "seed_run_id"}:
        raise ValueError("v23.2 input must contain run_id, sequence, seed_run_id")
    ...
    expected_config = {
        ...
        "domain_order": "V13/V19 local eligibility before complete-chain assembly",
    ...
    if manifest.get("selector_config") != expected_config:
        raise RuntimeError("selector-v23.2 configuration drift")
    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(prefix="fold-protein-v23-2-sealed-",
                                  dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v23_2(sequence, seed_record["states"])
        record = {
            "schema": "fold-protein-selected-states/v23.2", "status": "completed",
            ...
            "parent_states_retained_in_domains": result["parent_states_retained_in_domains"],
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v23.2", "status": "completed",
            "result_type": "cumulative development benchmark", "official_run": False,
            "execution": "locally closed complete domains before bidirectional global assembly",
            ...
            "segment_residues": SEGMENT_RESIDUES, "segment_count": result["segment_count"],
            "whole_chain_evaluations": result["whole_chain_evaluations"],
            "runtime_seconds": result["runtime_seconds"],
...
def main():
    ...
    parser.add_argument("manifest", type=Path); parser.add_argument("selector_input", type=Path)
    parser.add_argument("output_dir", type=Path); args = parser.parse_args()
    print(json.dumps(run_protocol_v23_2(
