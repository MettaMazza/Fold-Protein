#!/usr/bin/env python3
"""Run and immutably seal the binary-balanced selector-v7 route."""
...
try:
    from tools.blind_24_lattice_selector_v7 import (
        BINARY_MODE_COUNT, MODE_CAPACITY, MODE_NAMES, ORIENTATION_CONSTITUTION,
        ORIENTATION_MODES, forced_beam_width, select_state_path_v7)
...
def _load_input(path: Path) -> tuple[dict, bytes]:
    ...
    if set(value) != {"run_id", "sequence"}:
        raise ValueError("selector-v7 input must contain exactly run_id and sequence")
...
def run_protocol_v7(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v7":
        raise ValueError("unsupported selector-v7 manifest")
    ...
    expected_config = {
        ...
        "orientation_residues": ORIENTATION_CONSTITUTION["orientation_residues"],
        "overlap_residues": ORIENTATION_CONSTITUTION["overlap_residues"],
        "generated_modes": list(MODE_NAMES),
    }
    if manifest.get("selector_config") != expected_config:
        raise RuntimeError("selector-v7 configuration is not the forced census")

    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(prefix="fold-protein-v7-sealed-",
                                  dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v7(selector_input["sequence"])
        state_record = {
            "schema": "fold-protein-selected-states/v7",
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v7",
            "status": "completed",
            "execution": "sequence-only binary-balanced orientation selector output",
            "run_id": selector_input["run_id"],
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v7(
