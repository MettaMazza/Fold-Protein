#!/usr/bin/env python3
"""Run and seal V35 complete Protein boundary propagation."""
...
def run_protocol_v35(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v35":
        raise ValueError("unsupported selector-v35 manifest")
    if set(selector_input) != {"run_id", "sequence"}:
        raise ValueError("V35 input must contain exactly run_id and sequence")
    for relative, expected in manifest["source_sha256"].items():
        path = ROOT / relative
        if not path.is_file() or sha256_file(path) != expected:
            raise RuntimeError(f"V35 protocol source drift: {relative}")
    expected_config = {
        ...
        "quartet_residues": QUARTET_RESIDUES,
    ...
    if manifest.get("selector_config") != expected_config:
        raise RuntimeError("V35 selector configuration drift")

    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(prefix="fold-protein-v35-sealed-",
                                  dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        sequence = str(selector_input["sequence"]).upper()
        result = select_state_path_v35(sequence)
        record = {
            "schema": "fold-protein-selected-states/v35",
            "status": "completed",
            ...
            "quartet_transitions": result["quartet_transitions"],
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v35",
            "status": "completed",
            "result_type": "cumulative development benchmark",
            "official_run": False,
            "execution": "complete V2 boundary graph under the admitted V3 order",
            "run_id": selector_input["run_id"],
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v35(
