#!/usr/bin/env python3
"""Run and immutably seal the inter-window orientation selector-v5 route."""
...
try:
    from tools.blind_24_lattice_selector_v5 import (
        ORIENTATION_CONSTITUTION, forced_beam_width, select_state_path_v5)
...
def _load_input(path: Path) -> tuple[dict, bytes]:
    ...
    if set(value) != {"run_id", "sequence"}:
        raise ValueError("selector-v5 input must contain exactly run_id and sequence")
...
def run_protocol_v5(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v5":
        raise ValueError("unsupported selector-v5 manifest")
    ...
    if manifest.get("selector_config") != expected_config:
        raise RuntimeError("selector-v5 configuration is not the forced census")

    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(prefix="fold-protein-v5-sealed-",
                                  dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v5(selector_input["sequence"])
        state_record = {
            "schema": "fold-protein-selected-states/v5",
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v5",
            "status": "completed",
            "execution": "sequence-only inter-window orientation selector output",
            "run_id": selector_input["run_id"],
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v5(
