#!/usr/bin/env python3
"""Run and immutably seal selector v11 before target comparison."""
...
try:
    ...
    from tools.blind_24_lattice_selector_v11 import (
        CHARGE_CENSUS, CONSTITUTIONAL_OBJECTIVES, HYDROGEN_BOND_CENSUS,
        STERIC_CENSUS, select_state_path_v11)
    ...
    from blind_24_lattice_selector_v7 import (
        BINARY_MODE_COUNT, MODE_CAPACITY, MODE_NAMES)
    from blind_24_lattice_selector_v11 import (
        CHARGE_CENSUS, CONSTITUTIONAL_OBJECTIVES, HYDROGEN_BOND_CENSUS,
        STERIC_CENSUS, select_state_path_v11)
...
def run_protocol_v11(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v11":
        raise ValueError("unsupported selector-v11 manifest")
    ...
    if set(selector_input) != {"run_id", "sequence"}:
        raise ValueError(
            "selector-v11 input must contain exactly run_id and sequence")
    ...
    if manifest.get("selector_config") != expected_config:
        raise RuntimeError("selector-v11 configuration is not the forced census")

    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(prefix="fold-protein-v11-sealed-",
                                  dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v11(selector_input["sequence"])
        state_record = {
            "schema": "fold-protein-selected-states/v11",
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v11",
            "status": "completed",
            "execution": (
                "sequence-only symmetric ordinal selector with directional "
                "unit-capacity backbone donor/acceptor assembly"),
            "run_id": selector_input["run_id"],
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v11(
