#!/usr/bin/env python3
"""Selector v12: topology-separated backbone hydrogen-bond assembly.

V12 retains v10's complete balanced constitution and replaces v11's single
hydrogen-bond objective with independent alpha and inter-strand relations.
Both enter the same symmetric ordinal balance without a mixing weight.
"""
...
try:
    ...
    from backbone_hydrogen_bond_constitution_v2 import (
        TOPOLOGY_HYDROGEN_BOND_CENSUS,
        dimensionless_topology_hydrogen_bond_relations,
        topology_backbone_hydrogen_bond_relation)
...
CONSTITUTIONAL_OBJECTIVES = 13


def generated_prefix_relations_v12(sequence: str, state_path: list[int]):
    ...
    topology = dimensionless_topology_hydrogen_bond_relations(
        active_sequence, atoms)
    return tuple(base) + tuple(topology)


def generated_local_relations_v12(sequence: str, state_path: list[int]):
    base = generated_local_relations(sequence, state_path)
    prefix_length = len(state_path) + 1
    if prefix_length <= ORIENTATION_QUARTET:
        return tuple(base) + (0.0, 0.0)
    atoms = build_lookahead_prefix(sequence, state_path)
    # Alpha first becomes constructible in the terminal five-residue window.
    # Longer topology cannot exist inside that window and is evaluated by the
    # complete-prefix relation after local pruning.
    atoms = atoms[-3 * (ORIENTATION_QUARTET + 1):]
...
def select_state_path_v12(sequence: str) -> dict:
    ...
    for index in range(len(sequence) - 1):
        ...
        for path in beam:
            for state in active_candidates(index, len(sequence)):
                ...
                local = generated_local_relations_v12(sequence, list(candidate))
        ...
        for mode in MODE_NAMES:
            ...
            for _, _, candidate in eligible:
                complete = generated_prefix_relations_v12(
    ...
    final_raw = {}
    for path in beam:
        local = generated_local_relations_v12(sequence, list(path))
        complete = generated_prefix_relations_v12(sequence, list(path))
