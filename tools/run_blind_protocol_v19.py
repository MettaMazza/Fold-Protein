#!/usr/bin/env python3
"""Run and immutably seal selector v19 before target comparison."""
...
from tools.blind_24_lattice_selector_v19 import (
    ...
    TOPOLOGY_HYDROGEN_BOND_CENSUS, select_state_path_v19)
...
def run_protocol_v19(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v19":
        raise ValueError("unsupported selector-v19 manifest")
    ...
    if set(selector_input) != {"run_id", "sequence"}:
        raise ValueError("selector-v19 input must contain run_id and sequence")
    expected = {
        "beam_width": 24,
    ...
    if manifest.get("selector_config") != expected:
        raise RuntimeError("selector-v19 configuration is not the forced census")
    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(
        prefix="fold-protein-v19-sealed-", dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v19(selector_input["sequence"])
        state_record = {
            "schema": "fold-protein-selected-states/v19",
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v19",
            "status": "completed",
            "result_type": "cumulative development benchmark",
            "official_run": False,
            "execution": (
                "v13 local frontier with explicit graph exclusion at "
                "hierarchical complete-prefix assembly"),
            "run_id": selector_input["run_id"],
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v19(
