#!/usr/bin/env python3
"""Run and immutably seal the charge-aware selector-v8 route."""
...
def _load_input(path: Path) -> tuple[dict, bytes]:
    ...
    if set(value) != {"run_id", "sequence"}:
        raise ValueError("selector-v8 input must contain exactly run_id and sequence")
...
def run_protocol_v8(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v8":
        raise ValueError("unsupported selector-v8 manifest")
    ...
    if manifest.get("selector_config") != expected_config:
        raise RuntimeError("selector-v8 configuration is not the forced census")

    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(prefix="fold-protein-v8-sealed-",
                                  dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v8(selector_input["sequence"])
        state_record = {
            "schema": "fold-protein-selected-states/v8",
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v8",
            "status": "completed",
            "execution": "sequence-only charge-aware orientation selector output",
            "run_id": selector_input["run_id"],
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v8(
