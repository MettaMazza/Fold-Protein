#!/usr/bin/env python3
"""Run and immutably seal the signed-orientation selector-v6 route."""
...
try:
    from tools.blind_24_lattice_selector_v6 import (
        ORIENTATION_CONSTITUTION, ORIENTATION_MODES, forced_beam_width,
        select_state_path_v6)
...
def _load_input(path: Path) -> tuple[dict, bytes]:
    ...
    if set(value) != {"run_id", "sequence"}:
        raise ValueError("selector-v6 input must contain exactly run_id and sequence")
...
def run_protocol_v6(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v6":
        raise ValueError("unsupported selector-v6 manifest")
    ...
    expected_config = {
        ...
        "generated_modes": ["alpha", "beta"],
    }
    if manifest.get("selector_config") != expected_config:
        raise RuntimeError("selector-v6 configuration is not the forced census")

    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(prefix="fold-protein-v6-sealed-",
                                  dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v6(selector_input["sequence"])
        state_record = {
            "schema": "fold-protein-selected-states/v6",
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v6",
            "status": "completed",
            "execution": "sequence-only signed-orientation selector output",
            "run_id": selector_input["run_id"],
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v6(
