#!/usr/bin/env python3
"""Run and immutably seal selector v22 before target comparison."""
...
from tools.blind_24_lattice_selector_v22 import (
    CHARGE_CENSUS, CONSTITUTIONAL_OBJECTIVES, CONTACT_DEFICIT_CENSUS,
    HARD_EXCLUSION_STRATA, HYDROGEN_BOND_CENSUS,
    SIDECHAIN_GRAPH_SPATIAL_CENSUS, STERIC_CENSUS,
    TOPOLOGY_HYDROGEN_BOND_CENSUS, select_state_path_v22)
...
def run_protocol_v22(manifest_path: Path, input_path: Path,
    ...
    if manifest.get("schema") != "fold-protein-blind-selector/v22":
        raise ValueError("unsupported selector-v22 manifest")
    ...
    if set(selector_input) != {"run_id", "sequence"}:
        raise ValueError("selector-v22 input must contain run_id and sequence")
    expected = {
        "beam_width": 24, "binary_mode_count": BINARY_MODE_COUNT,
        "mode_capacity": MODE_CAPACITY, "orientation_residues": 4,
        ...
        "sidechain_graphs": 20, "local_graph_pruning": False,
        "contact_inner_fold_denominator": 2,
        "contact_outer_step_units": 1,
        "contact_stratum": "possible_atom_pairs_minus_contacts",
        "spatial_non_neighbour_separation": 2,
    }
    if manifest.get("selector_config") != expected:
        raise RuntimeError("selector-v22 configuration is not the forced census")
    output_dir.parent.mkdir(parents=True, exist_ok=True)
    stage = Path(tempfile.mkdtemp(
        prefix="fold-protein-v22-sealed-", dir=output_dir.parent))
    try:
        (stage / "selector_input.json").write_bytes(input_raw)
        result = select_state_path_v22(selector_input["sequence"])
        state_record = {
            "schema": "fold-protein-selected-states/v22",
            "run_id": selector_input["run_id"],
            "sequence": selector_input["sequence"].upper(),
            "states": result["states"], "score_trace": result["score_trace"],
            ...
            "sidechain_graph_contact_census": result["sidechain_graph_contact_census"],
        ...
        seal = {
            "schema": "fold-protein-blind-seal/v22", "status": "completed",
            "result_type": "cumulative development benchmark",
            "official_run": False,
            "execution": "V13-local explicit contact-deficit assembly hierarchy",
            "run_id": selector_input["run_id"],
            ...
            "binary_mode_count": BINARY_MODE_COUNT, "mode_capacity": MODE_CAPACITY,
            "constitutional_objectives": CONSTITUTIONAL_OBJECTIVES,
            "hard_exclusion_strata": list(HARD_EXCLUSION_STRATA),
            "local_graph_pruning": False, "charge_census": CHARGE_CENSUS,
...
def main() -> None:
    ...
    print(json.dumps(run_protocol_v22(
