...
The project does not assign validity by agent labels. The engine traces admitted forcing and derivation to the One and halts on violation. A computational relation is either engine-closed, an explicitly named forward-forcing constitution, or OPEN; exact execution is never silently promoted between those categories. The 24-lattice construction contains zero trained weights, zero neural networks, and no fitted force field. Its finite states and coordinate construction are inspectable end to end.

### 3.1 Blind structural predictions and empirical agreement

The exact-fraction Protein module checks the principal secondary-structure coordinates independently of any target-specific sequence path. Fractions are measured on the complete 360° circle; the assembled coordinate forms remain OPEN for the corpus's finished form guards:
...
1. v5 executes the checked four-residue relation `c + c - b = 4` and inter-window topology on a sealed 76-residue sequence. Its full output reached **TM 0.1232111976 / 8.3625317712 Å dRMSD**, with `DKE` at **0.9991809285 local TM**, `DKEG` at **0.8460707854**, and `QDKEG` at **0.3332405586**. The assembled relation remains OPEN for the complete form guard.
...
12. v29 is an OPEN agent-authored sequence-only ordinal spanning-tree architecture over four-residue bodies. Its L32 output reaches **TM 0.0499243401 / 4.5466887676 Å dRMSD** and its L24 dRMSD reaches **5.4137342407 Å**. V30 is an OPEN degree-two-path architecture whose L32 output reaches **TM 0.0502723476 / 4.9045982349 Å dRMSD**. V31-V33 are likewise OPEN development architectures testing a degree frontier and two consensus orders. V33's completed L76 development output reproduces V29 byte-exactly at **TM 0.0938366517 / 8.7716502163 Å dRMSD**, 26.96% lower in TM and 26.00% poorer in dRMSD than V28. Their sealed measurements and provenance remain valid, but none of those assembled forms is promoted as forced or derived. The rows do not authorize a further selector; any new relation must first pass the complete corpus admission guard.
...
No theoretical wall is established at this stage. The protected construction proves that the same 24×24 lattice contains a 76-residue ubiquitin trace at **0.9891211351 TM / 0.2608575408 Å dRMSD**; the exact-fraction module checks canonical alpha-helix and beta-sheet coordinates; the blind engine has completed the entire sequence; and the continuation has successively activated inter-window, orientation, charge, side-chain, and balanced assembly implementations. The present absence of finished form closure is a defined derivation task, not evidence of an obstruction.
...
Fold Protein has secured Super Parity through a 0.9891211351 TM, zero-parameter, replayable finite construction. Its exact-fraction structural module checks the canonical alpha-helix and beta-sheet dihedral coordinates against exact rational angles; finished form closure for those coordinate forms is explicitly OPEN. The SFT sequence engine has completed target-isolated, pre-comparison-sealed blind 76-residue ubiquitin predictions and produced highly accurate local geometry. V13 advances whole-chain TM by **57.49%** over V10 while improving every 48-residue window. V25 reaches **0.1291502547 TM / 7.1461955341 Å dRMSD**. V26.1 advances complete-chain dRMSD to **6.7648477959 Å** while retaining **0.1287565476 TM**. V28 improves both V27 complete-length measures to **0.1284665482 TM / 6.9615228794 Å dRMSD**. The OPEN V29/V30 architectures establish a large measured L32 advance. These positive applied results remain intact while their selector forms undergo corpus-standard derivation closure. Levinthal's astronomical search is replaced by directed fold descent and finite rational geometry. The secured construction and accumulated blind evidence establish no theoretical wall.
